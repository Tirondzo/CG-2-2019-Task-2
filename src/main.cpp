  ...
  for (int x = 0; x < cols - 1; ++x)
  {
    for (int z = 0; z < rows - 1; ++z)
    {
      ...
      else // иначе нам достаточно двух индексов, чтобы добавить два треугольника
      {
        ...
        if (z == rows - 2)
          indices_vec.push_back(primRestart); // если мы дошли до конца строки, вставляем primRestart, чтобы обозначить переход на следующую строку
      }
    }
  }
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    ShaderProgram terrain_program(terrain_shaders);
    GL_CHECK_ERRORS;
    ...
    ShaderProgram debug_program(debug_shaders);
    GL_CHECK_ERRORS;
